...
# In preivous chapters we've seen a surprising relationship between the quality of diamonds and their price: 
# low quality diamonds (poor cuts, bad colours, and inferior clarity) have highter prices.
ggplot(diamonds, aes(cut, price)) + geom_boxplot()
ggplot(diamonds, aes(color, price)) + geom_boxplot()
ggplot(diamonds, aes(clarity, price)) + geom_boxplot()

# Note that the worst diamond color is J (slightly yellow), and the worst clarity is I1 (inclusions visible to the nakes eye).
...
# The weight of the diamond is the single most important factor for determining the price of the diamond, and lower quality daimonds tend to be large.
...
# 2. Log-transform the carat and price variabels
diamonds2 <- diamonds %>% 
  filter(carat <= 2.5) %>% 
...
# Let's take the next step and remove that strong linear pattern. We first make the pattern explicit by fitting a model:
mod_diamond <- lm(lprice ~ lcarat, data = diamonds2)

# Then we look at what the model tells us about the data. Note that I back transform the predictions, 
# undoing the log transformation, so I can overlay the predictions on the raw data:
grid <- diamonds2 %>% 
...
# That tells us something interesting about our data. If we believe our model, then the large diamonds are much cheaper then expected.
# This is probably because no diamond in this dataset costs more than $19,000.

# Now we can look at the residuals, which verifies that we've successfuly removed the strong linear pattern:
diamonds2 <- diamonds2 %>% 
